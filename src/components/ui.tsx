...
export function Cover({src,title,className="",radius="rounded-md",priority=false}:{src?:string|null;title:string;className?:string;radius?:string;priority?:boolean}) {
  ...
  return <div className={`book-cover relative aspect-[2/3] self-start overflow-hidden bg-fill shadow-raise ${radius} ${className}`}>
    {src&&failedSrc!==src ? <Image src={src} alt="" fill priority={priority} sizes="(max-width:768px) 25vw, 160px" className="object-cover" onError={()=>setFailedSrc(src)} /> : <div className="cover-fallback absolute inset-0 flex flex-col justify-between border-l-[3px] border-black/10 px-3 py-4"><span className="line-clamp-4 text-[12px] font-medium leading-relaxed text-muted">{title}</span><Icon name="book" className="h-4 w-4 shrink-0 text-faint" /></div>}
